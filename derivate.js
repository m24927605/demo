...
;(async()=>{
    try{
			const phrase = bip39.generateMnemonic() // this will generate a random mnemonic words
			...
			const xpubString = btcAccount0.neutered().toBase58()
			console.log('[xpub]',xpubString)
			const btcAddress0FromXpub = bitcoin.HDNode.fromBase58(xpubString)
			...
			const btcAddress0 = btcAddress0FromXpub.derivePath("0/0").keyPair.getAddress()
			const btcAddress1 = btcAddress0FromXpub.derivePath("0/1").keyPair.getAddress()
			const btcAddress2 = btcAddress0FromXpub.derivePath("0/2").keyPair.getAddress()
			const btcAddress3 = btcAddress0FromXpub.derivePath("0/3").keyPair.getAddress()
